...
**Honestly:** `R2Storage` is written but has never run against a live bucket,
because this exercise has no bucket. A test asserts both classes expose
identical signatures, so drift fails locally rather than the first time someone
flips the env var, but that is not the same as having used it.
...
- **Publish dry-run diff** and **audit log of who changed what**. Both are real
  features rather than polish, and neither changes whether the pipeline is
  correct. Publish runs already record who, when, counts and outcome.
...
drift, then 147 pytest tests against a real Postgres), `build` (all three
...
147, concentrated on what is risky rather than spread evenly:
